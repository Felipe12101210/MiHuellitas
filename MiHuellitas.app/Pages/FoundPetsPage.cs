...
public class FoundPetsPage : ContentPage
{
    ...
    public FoundPetsPage()
    {
        ...
        if (_catalog is null)
        {
            layout.Children.Add(new Label { Text = "Ocurrió un error al cargar los reportes. Intenta más tarde." });
        }
        else
        {
            layout.Children.Add(new Label { Text = "Ayuda", Style = Res("Eyebrow") });
            layout.Children.Add(new Label { Text = "Encontradas", Style = Res("PageTitle") });
    ...
    private void BuildFilterOptions()
    {
        ...
        Populate(_speciesPicker, _catalog!.FoundPets.Select(p => p.Species), "Todas");
        Populate(_breedPicker, _catalog.FoundPets.Select(p => p.Breed), "Todas");
        Populate(_locationPicker, _catalog.FoundPets.Select(p => p.Location), "Todas");
        Populate(_statusPicker, _catalog.FoundPets.Select(p => p.Status.ToString()), "Todos");
    ...
    private void BuildList()
    {
        ...
        var filtered = _catalog.FoundPets
        ...
        if (!filtered.Any())
        {
            _itemsContainer.Children.Add(new Label { Text = "No se encontraron reportes de mascotas encontradas.", HorizontalOptions = LayoutOptions.Center });
            return;
        }
